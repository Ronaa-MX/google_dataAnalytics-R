...
```{r}
example_df <- bookings_df %>%
  mutate(guests = adultos, `nios`, `bebs`)

head(example_df) %>% filter(reservation_status == "canceled")
```
...
example_df <- bookings_df%>% filter(is_canceled == 1 ) %>% summarize(number_canceled = n(), average_lead_time = mean(lead_time))
